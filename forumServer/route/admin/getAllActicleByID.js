...
const getAllActicleByID = async (req,res)=>{
    ...
    // 当前页码
    let pagenum = req.body.queryInfo.pagenum
    // 当前每页显示多少条数据
    let pagesize = req.body.queryInfo.pagesize 
    // 数据库中含有多少数据
    let count = await Article.countDocuments({author})
    // 计算总页数：总数据/每页显示多少，向上取整
    let pages = Math.ceil(count/pagesize)
    let start = (pagenum -1)*pagesize
    // 结构出传递的查询关键字
    let searchVal = req.body.queryInfo.query
    // 设置返回数据
    ...
    // 查询数据库中的所有数据
    let data = await Article.find({author}).populate({ path: 'author', select: 'username _id' }).limit(pagesize).skip(start).select('-password -token').sort('-publishtime')
    // 整理数据格式
