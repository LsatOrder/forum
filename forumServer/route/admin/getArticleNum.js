...
const getArticleNum = async (req,res)=>{
    ...
    // 查询对应的数据
    let day1data = await Article.countDocuments({ "publishtime" : { $lte: today , $gt: day1 }})
    let day2data = await Article.countDocuments({ "publishtime" : { $lte: day1 , $gt: day2 }})
    let day3data = await Article.countDocuments({ "publishtime" : { $lte: day2 , $gt: day3 }})
    let day4data = await Article.countDocuments({ "publishtime" : { $lte: day3 , $gt: day4 }})
    let day5data = await Article.countDocuments({ "publishtime" : { $lte: day4 , $gt: day5 }})
    let day6data = await Article.countDocuments({ "publishtime" : { $lte: day5 , $gt: day6 }})
    let day7data = await Article.countDocuments({ "publishtime" : { $lte: day6 , $gt: day7 }})
